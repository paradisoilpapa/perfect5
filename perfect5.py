...
def _v335bt_purchase_lines(final_order, profile):
    ...
    def _v335dc_restore_himo_lines(_base_order, _axis_car, _line_def_map):
        ...
        v335df ヒモ選定仕様
        1) 開催日KOで確定した軸は変更しない。
        2) 軸がライン所属なら、同ラインの他車のうち
           「開催日KO最終順位が最上位の1車」を必ずヒモに確保する。
           2車ラインなら相方を、KO4位以下でも必ず確保する。
        3) 残り2枠は開催日KO最終順位の上位から、軸・確保済み車を除いて採用する。
        4) 軸が単騎ならライン保護枠なしで、KO最終順位上位3車を採用する。
        5) 次候補ラインの丸ごと復元・上流/下流の連結は行わない。
        """
        _axis = int(_axis_car)
