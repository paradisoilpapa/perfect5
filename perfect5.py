...
# ・展開2位の3連複3車目は、展開1位の逆転2車単の頭（展開1位最終2位）を採用。
# ・その車が展開2位TOP2と重複する場合は、展開1位逆転2車単のもう一方（展開1位最終1位）を採用。両方重複なら生成不可。
# ・展開3位ワイドは購入から廃止。予想本体・流れ比率・ポイントアップ・開催日KOは変更しない。
# ・合計4点を基本とする。
# v335ey（展開順位別・券種ダウン4点版）
# ・展開1位：流れ別最終着順TOP3の3連複1点＋TOP2逆転2車単1点。
# ・展開2位：流れ別最終着順TOP2の2車複1点。
# ・展開3位：流れ別最終着順1位を軸に、同一ラインを除く最上位1車とのワイド1点。
# ・想定展開が下がるほど券種を 3連複/2車単 → 2車複 → ワイド と下げる。合計4点を基本とする。
...
# v335em（推奨表示整理版）
# ・推奨候補は「評価軸　★順位　平均総合点」を先に表示し、その下に券種と買い目を表示する。
# ・総合点計算、★順位判定、買い目生成、その他予想ロジックはv335elから変更しない。
# v335ek（評価1軸・裏目2車単追加版）
# ・評価1軸3連単 1-2-345 に 2車単2-1を1点追加。
# ・評価1軸3連単 1-3-245 に 2車単3-1を1点追加。
# ・追加2車単は★★★★～★の順位判定・平均総合点には含めない。その他ロジックは変更しない。
# v335ef（推奨購入4点固定・★本線版）
...
# 既存の推奨購入・★判定・総合点・検証買い目は変更しない。
# v335ec（圧縮買目・整合修正版）
# 3連単1→2型 → 対抗2車単は評価2軸 → 圧縮は2車複2-3・2-4・2-6（3点）。
# 3連単2→1型 → 対抗2車単は評価1軸 → 圧縮は2車複1-3・1-4・2-4・2-6（4点）。
# 1↔2の安い2車単は圧縮買目から削除。その他ロジックは変更しない。
# v335eb（圧縮買目 修正版）
...
# ・検証買い目は既存の推奨購入、平均総合点、★判定には一切加算・使用しない。
...
def _v335es_flow_top2_purchase_lines(profile, v_order=None):
    ...
    if not _ratio_map:
        try:
            ...
            _tot = sum(_vals.values())
            _ratio_map = (
                {k: float(v) / float(_tot) for k, v in _vals.items()}
    ...
    for _style in ("順流", "逆流", "渦"):
        ...
        _rows.append({
            ...
            "order": tuple(int(c) for c in _final),
            "diag": dict(_diag or {}),
    ...
    def _rank_map(_order):
        return {int(c): int(i) for i, c in enumerate((_order or tuple()), start=1)}

    _rank1 = _rank_map(_order1)
    _rank2 = _rank_map(_order2)
    _rank3 = _rank_map(_order3)

    _cars = []
    for _seq in (_order1, _order2, _order3, tuple(int(x) for x in (v_order or tuple()))):
    ...
    def _push_line(_raw):
        try:
            if isinstance(_raw, (list, tuple, set)):
                _xs = [int(x) for x in _raw if str(x).isdigit()]
            else:
                _xs = [int(ch) for ch in str(_raw) if ch.isdigit() and ch != "0"]
            _uniq = []
            for _x in _xs:
                if _x not in _uniq:
                    _uniq.append(_x)
            if _uniq and _uniq not in _line_groups:
                _line_groups.append(_uniq)
        except Exception:
            pass
            ...
            if _axis in _line and _car in _line:
                try:
                    ...
                    return {
                        "same": True,
                        "behind": _ic > _ia,
                        "adjacent_behind": _ic == _ia + 1,
                        "distance": abs(_ic - _ia),
                    }
                except Exception:
...
def _v335bt_purchase_lines(final_order, profile):
    ...
    _out = [
        ...
        *[_final_exacta_line(_ticket) for _ticket in _final_exacta_tickets],
        ...
        f"※最終着順予想1位→2位（{_pred1}-{_pred2}）の決着は低配当になりやすいため、推奨買い目には採用していません。リアルタイムで購入する場合は、オッズをご確認ください。",
    ]
