...
if len(trioC_list) > 0:
    ...
    for t in trioC_list:
        ...
        rows.append({
            "買い目": f"{t[0]}-{t[1]}-{t[2]}",
            "p(想定的中率)": round(p, 4),
            # EV下限=1/p をそのまま出す（表示側で帯に変換）
            "必要オッズ(=1/p)": "-" if cnt==0 else round(1.0/p, 2)
        })
    ...
    if O_combo is not None and len(Sset) > 0:
        st.caption(f"三連複バスケット合成オッズ（下限基準）：**{O_combo:.2f}倍** / 相手集合S：{sorted(Sset)}")
