...
try:
   ...
   # === ライン想定FR（順流/渦/逆流 + その他） ===
try:
    # 上流の flow / bets / line_fr_map / all_lines / _free_fmt_nums がある前提
    # ただし無くても落とさない
    ...
    try:
        note_sections.append(f"[LINE FR ERROR] {type(e).__name__}: {e}")
    except Exception:
        pass
    ...
    #    ※ “母集団（KO入力）” と “予想で使うスコア” を同じに揃える（score_map）
    # =========================================================
    ...
    if vals_pos:
        ...
        floor = max(p10, min(vals_pos_sorted))
    else:
        floor = 1e-6
    ...
    def _infer_line_zone(ln):
        ...
        # flow由来があれば優先
        if FR_line and _line_key(FR_line) == k:
            return "順流"
        if U_line and _line_key(U_line) == k:
            return "逆流"
        if VTX_line and _line_key(VTX_line) == k:
            return "渦"
