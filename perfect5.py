...
def _v273_normalize_mark(mark):
    mk = str(mark or "").strip()
    if mk == "○":
        return "〇"
    if mk == "▲":
        return "△"
    return mk if mk in _V273_MARK_ORDER else ""


def _v273_normalize_ratio_map(flow_ratio_map):
    styles = ("順流", "渦", "逆流")
    clean = {}
    try:
        for style in styles:
            v = float((flow_ratio_map or {}).get(style, 0.0) or 0.0)
            clean[style] = v if math.isfinite(v) and v >= 0.0 else 0.0
    except Exception:
        clean = {style: 0.0 for style in styles}
    total = sum(clean.values())
    if total <= 0.0:
        return {style: 1.0 / 3.0 for style in styles}
    return {style: float(clean[style]) / total for style in styles}


def _v273_unique_active_cars(style_seq_map, hit_map=None, myoumi_map=None, mark_map=None):
    out = []
    seen = set()
    for style in ("順流", "渦", "逆流"):
        for x in ((style_seq_map or {}).get(style, []) or []):
            ...
            if car not in seen:
                seen.add(car)
                out.append(car)
    for obj in (hit_map or {}, myoumi_map or {}, mark_map or {}):
        try:
            keys = obj.keys()
        except Exception:
            keys = []
...
def _v273_axis_ranking(style_seq_map, flow_ratio_map, hit_map=None, myoumi_map=None, ko_map=None, mark_map=None):
    """三流れの着内安定性を最優先にしたヴェロビ軸順位。"""
    ratios = _v273_normalize_ratio_map(flow_ratio_map)
    active = _v273_unique_active_cars(style_seq_map, hit_map, myoumi_map, mark_map)
    if not active:
        return []

    normalized_marks = {}
    for car, mark in dict(mark_map or {}).items():
        try:
            ci = int(car)
        except Exception:
            continue
        ...
        if mk:
            normalized_marks[ci] = mk

    seqs = {}
    fallback_rank = len(active) + 1
    for style in ("順流", "渦", "逆流"):
        seq = []
        for x in ((style_seq_map or {}).get(style, []) or []):
            try:
                ...
                continue
            if car in active and car not in seq:
                seq.append(car)
        for car in active:
            if car not in seq:
                seq.append(car)
    ...
    for car in active:
        ...
        for style in ("順流", "渦", "逆流"):
            seq = seqs.get(style, [])
            try:
                rank = seq.index(car) + 1
            except ValueError:
                rank = fallback_rank
            rank_by_style[style] = int(rank)
            ratio = float(ratios.get(style, 0.0) or 0.0)
            weighted_rank += ratio * float(rank)
            if rank <= 3:
                top3_support += ratio
                flow_top3_count += 1
            if rank <= 2:
        ...
        try:
            hit = float((hit_map or {}).get(car, 0.0) or 0.0)
        except Exception:
            hit = 0.0
        try:
            myoumi = float((myoumi_map or {}).get(car, 0.0) or 0.0)
        except Exception:
            myoumi = 0.0
        try:
            ko = float((ko_map or {}).get(car, 0.0) or 0.0)
        except Exception:
            ko = 0.0

        rows.append({
            "car": int(car),
            "mark": normalized_marks.get(int(car), ""),
            "top3_support": float(top3_support),
            "top2_support": float(top2_support),
            "win_support": float(win_support),
            "weighted_rank": float(weighted_rank),
            "flow_top3_count": int(flow_top3_count),
            "rank_by_style": dict(rank_by_style),
            "hit": float(hit),
            "myoumi": float(myoumi),
            "ko": float(ko),
        })

    # 合意順：着内支持率 → 加重平均着順 → 1着支持率 → 的中評価 → KO。
    rows.sort(key=lambda row: (
        float(row.get("top3_support", 0.0)),
        -float(row.get("weighted_rank", 99.0)),
        float(row.get("win_support", 0.0)),
        float(row.get("hit", 0.0)),
        float(row.get("ko", 0.0)),
        float(row.get("myoumi", 0.0)),
        -int(row.get("car", 99)),
    ), reverse=True)
    for idx, row in enumerate(rows, start=1):
        row["axis_rank"] = int(idx)
    return rows
...
def _v273_classify_flow_top3(top3, mark_map):
    """競馬本の人気構造を、AI印あり／無印の配置へ置換したA～E分類。"""
    marks = []
    for car in list(top3 or [])[:3]:
        marks.append(_v273_normalize_mark((mark_map or {}).get(int(car), "")))
    while len(marks) < 3:
        marks.append("")
    marked_positions = [idx + 1 for idx, mk in enumerate(marks) if mk]
    marked_count = len(marked_positions)

    if marked_count == 3:
        ranks = [_V273_MARK_ORDER.get(mk, 99) for mk in marks]
        if ranks == sorted(ranks):
            return "E", "人気3車・AI順"
        return "D2", "人気3車・順番入替"
    if marked_count == 2:
        unmarked_pos = next((idx + 1 for idx, mk in enumerate(marks) if not mk), 3)
        if unmarked_pos == 3:
            return "D1", "人気2車が上位・無印3着"
        return "C2", f"無印が{unmarked_pos}着まで進出"
    if marked_count == 1:
        marked_pos = marked_positions[0]
        if marked_pos == 1:
            return "C1", "人気1車が1着・無印2席"
        return "B", f"人気車は{marked_pos}着の1席のみ"
    return "A", "上位3席すべてAI無印"


def _v273_base_class(label):
    s = str(label or "")
    return s[:1] if s[:1] in _V273_CLASS_ORDER else "A"


def _v273_partner_profiles(axis, ranking_rows, style_seq_map, flow_ratio_map):
    ratios = _v273_normalize_ratio_map(flow_ratio_map)
    rank_map = {int(row.get("car")): row for row in (ranking_rows or [])}
    active = sorted(rank_map.keys())
    seqs = {}
    for style in ("順流", "渦", "逆流"):
        seq = []
        for x in ((style_seq_map or {}).get(style, []) or []):
    ...
    for car in active:
        ...
        for style in ("順流", "渦", "逆流"):
            ...
            if ar <= 3 and cr <= 3:
                ...
                co_flow_count += 1
                styles.append(style)
                order_signs.append(-1 if ar < cr else 1)
            if ar <= 2 and cr <= 2:
                co_top2_ratio += ratio
                top2_flow_count += 1
        consistent = bool(order_signs) and len(set(order_signs)) == 1
        direction = ""
        if consistent:
            direction = "軸先着" if order_signs[0] == -1 else "相手先着"
        row = dict(rank_map.get(car, {}) or {})
        row.update({
            "co_top3_ratio": float(co_top3_ratio),
            "co_top2_ratio": float(co_top2_ratio),
            "co_flow_count": int(co_flow_count),
            "top2_flow_count": int(top2_flow_count),
            "order_consistent": bool(consistent),
            "direction": direction,
            "co_styles": tuple(styles),
        })
        out.append(row)

    out.sort(key=lambda row: (
        float(row.get("co_top3_ratio", 0.0)),
        float(row.get("co_top2_ratio", 0.0)),
        1 if _v273_normalize_mark(row.get("mark")) else 0,
        float(row.get("top3_support", 0.0)),
        -float(row.get("weighted_rank", 99.0)),
        float(row.get("hit", 0.0)),
        float(row.get("ko", 0.0)),
...
def _v273_flow_top3_records(style_seq_map, flow_ratio_map, mark_map):
    ratios = _v273_normalize_ratio_map(flow_ratio_map)
    records = []
    for style in ("順流", "渦", "逆流"):
        seq = []
        for x in ((style_seq_map or {}).get(style, []) or []):
            ...
            if car not in seq:
                seq.append(car)
        top3 = tuple(seq[:3])
        label, reason = _v273_classify_flow_top3(top3, mark_map)
        records.append({
            "style": style,
            "ratio": float(ratios.get(style, 0.0) or 0.0),
            "top3": top3,
            "class": label,
            "base_class": _v273_base_class(label),
            "reason": reason,
        })
    return records


def _v273_unique_flow_trios(flow_records):
    out = []
    seen = set()
    for rec in sorted(list(flow_records or []), key=lambda x: float(x.get("ratio", 0.0)), reverse=True):
        top3 = tuple(int(x) for x in (rec.get("top3") or []) if str(x).isdigit())
        ...
        if key in seen:
            continue
        seen.add(key)
        out.append({"key": key, "style": rec.get("style", ""), "ratio": rec.get("ratio", 0.0)})
    return out


def _v276_axis_top2_flow_count(axis_row):
    count = 0
    for rank in dict((axis_row or {}).get("rank_by_style", {}) or {}).values():
        try:
            if int(rank) <= 2:
                count += 1
        except Exception:
...
def _v276_select_box4(ranking_rows, flow_records):
    """各流れの1・2着組を最も多く包含する4車を選ぶ。"""
    rows = [dict(row) for row in (ranking_rows or [])]
    active = [int(row.get("car")) for row in rows if str(row.get("car", "")).isdigit()]
    rank_map = {int(row.get("car")): row for row in rows if str(row.get("car", "")).isdigit()}
    if len(active) <= 4:
        return tuple(active)

    best_combo = None
    best_key = None
    for combo in combinations(active, 4):
    ...
    selected = set(best_combo or tuple(active[:4]))
    return tuple(int(row.get("car")) for row in rows if int(row.get("car")) in selected)


def _v276_select_trio_partners(axis, ranking_rows, flow_records):
    """三流れの相手席を分散して残す4車を選ぶ。"""
    rows = [dict(row) for row in (ranking_rows or []) if int(row.get("car", -1)) != int(axis)]
    ...
    for combo in combinations(active, 4):
        ...
        flow_count = 0
        for rec in (flow_records or []):
            top3 = [int(x) for x in ((rec or {}).get("top3", tuple()) or tuple())]
            if int(axis) not in top3:
                continue
            ratio = float((rec or {}).get("ratio", 0.0) or 0.0)
            others = [x for x in top3 if x != int(axis)]
            hits = len(chosen.intersection(others))
            if hits:
                any_flow_cover += ratio
                flow_count += 1
            seat_cover += ratio * float(hits)
            if len(others) == 2 and set(others).issubset(chosen):
                full_flow_cover += ratio
        ...
        key = (
            int(flow_count),
            float(any_flow_cover),
            float(full_flow_cover),
            float(seat_cover),
            int(marked),
            float(top3_sum),
            float(top2_sum),
...
def _v276_choose_pattern(axis_row, ranking_rows, partner_rows, flow_records):
    """A～Eを、軸の三流れ安定度とAI印から機械的に決める。"""
    axis_mark = _v273_normalize_mark((axis_row or {}).get("mark"))
    axis_top3_count = int((axis_row or {}).get("flow_top3_count", 0) or 0)
    axis_top2_count = _v276_axis_top2_flow_count(axis_row)
    axis = int((axis_row or {}).get("car"))

    core = _v276_order_core(axis, ranking_rows, partner_rows)
    if axis_mark == "◎" and len(core) == 4:
        return "E", "ORDER_CORE", "3連単12-123-123＋3連複1-2-34", "堅軸に加え、上位2車の核が三流れで共通"

    if axis_mark == "◎":
        return "D", "WIN_AXIS", "2車単◎→全", "ヴェロビ軸順位1位とAI◎が一致した堅軸"

    # どれか一つの流れで3着外へ消えるなら、軸固定をやめる。
    if axis_top3_count < 3:
        return "A", "BOX4", "2車複4車BOX", "軸が三流れ共通の着内候補にならず、連対候補を4車へ集約"

    if axis_top2_count >= 2:
        return "C", "PAIR_AXIS", "2車複◎-全", "軸は三流れで着内し、複数流れで連対するが1着固定はしない"

    return "B", "TRIO_AXIS", "3連複◎-4車-4車", "軸は三流れすべてで着内するが、連対位置と相手が分散"


def _v276_build_pattern_tickets(
    pattern_code,
    axis,
    ranking_rows,
    partner_rows,
    ...
    """合意済み5パターンを最大6点で生成する。ワイドは使わない。"""
    max_tickets = max(0, int(max_tickets or 0))
    groups = []
    all_cars = [int(row.get("car")) for row in (ranking_rows or [])]
    others = [car for car in all_cars if car != int(axis)]

    if pattern_code == "BOX4":
        box4 = list(_v276_select_box4(ranking_rows, flow_records))[:4]
        vals = [f"{min(a,b)}-{max(a,b)}" for a, b in combinations(box4, 2)]
        if vals:
            groups.append(("2車複", vals[:max_tickets]))
        return groups

    if pattern_code == "TRIO_AXIS":
        partners4 = list(_v276_select_trio_partners(axis, ranking_rows, flow_records))[:4]
        vals = []
        for a, b in combinations(partners4, 2):
            trio = tuple(sorted((int(axis), int(a), int(b))))
            vals.append("-".join(str(x) for x in trio))
        if vals:
            groups.append(("3連複", vals[:max_tickets]))
        return groups

    if pattern_code == "PAIR_AXIS":
        vals = [f"{min(int(axis), int(car))}-{max(int(axis), int(car))}" for car in sorted(others)]
        if vals:
            groups.append(("2車複", vals[:max_tickets]))
        return groups

    if pattern_code == "WIN_AXIS":
        vals = [f"{int(axis)}→{int(car)}" for car in sorted(others)]
        if vals:
            groups.append(("2車単", vals[:max_tickets]))
        return groups
    ...
    if pattern_code == "ORDER_CORE":
        ...
        if len(core) != 4:
            return groups
        c1, c2, c3, c4 = [int(x) for x in core]
        santan = [
            f"{c1}→{c2}→{c3}",
            f"{c1}→{c3}→{c2}",
            f"{c2}→{c1}→{c3}",
        ...
        trio1 = "-".join(str(x) for x in sorted((c1, c2, c3)))
        trio2 = "-".join(str(x) for x in sorted((c1, c2, c4)))
        groups.append(("3連単", santan[:4]))
        groups.append(("3連複", [trio1, trio2]))
        return groups

    return groups


def _v273_build_three_layer_trial_plan(
    style_seq_map,
    flow_ratio_map,
    mark_map,
    hit_map,
    ...
    *,
    current_ticket="",
    current_santan_tickets=None,
    current_support_trio_rows=None,
    max_tickets=6,
):
    ranking = _v273_axis_ranking(
        style_seq_map,
        flow_ratio_map,
        hit_map=hit_map,
        myoumi_map=myoumi_map,
        ko_map=ko_map,
        mark_map=mark_map,
    )
    if not ranking:
    ...
    himo = [
        dict(row) for row in ranking[1:]
        if _v273_normalize_mark(row.get("mark")) and float(row.get("top3_support", 0.0) or 0.0) > 0.0
    ]
    lower_unmarked = [
        dict(row) for row in ranking[1:]
        if not _v273_normalize_mark(row.get("mark")) and float(row.get("top3_support", 0.0) or 0.0) > 0.0
    ]
    excluded = [
        int(row.get("car")) for row in ranking[1:]
        if float(row.get("top3_support", 0.0) or 0.0) <= 0.0
    ]

    flow_records = _v273_flow_top3_records(style_seq_map, flow_ratio_map, normalized_marks)
    class_share = {k: 0.0 for k in ("A", "B", "C", "D", "E")}
    ...
    partners = _v273_partner_profiles(axis, ranking, style_seq_map, flow_ratio_map)
    pattern_label, pattern_code, family, family_reason = _v276_choose_pattern(
        axis_row,
        ranking,
        partners,
        flow_records,
    )
    tickets = _v276_build_pattern_tickets(
        pattern_code,
        axis,
        ranking,
        partners,
    ...
    ticket_count = sum(len(items) for _, items in tickets)

    return {
        "ranking": tuple(ranking),
        "axis": axis,
        "axis_row": axis_row,
        "axis_mark": normalized_marks.get(axis, ""),
        "axis_type": _v273_axis_type(normalized_marks.get(axis, "")),
        "himo": tuple(himo),
        "lower_unmarked": tuple(lower_unmarked),
        "excluded": tuple(excluded),
        "flow_records": tuple(flow_records),
        "class_share": dict(class_share),
...
def _v273_format_three_layer_trial_block(plan):
    """v276: パターンA～Eと三層・買い目だけを簡潔表示。既存詳細は維持する。"""
    ...
    try:
        ...
        if flow_items:
            ...
            _v273_trial_plan = _v273_build_three_layer_trial_plan(
                ...
                current_ticket=_recommended_ticket,
                current_santan_tickets=_santan_tickets,
                current_support_trio_rows=_supplement_trio_rows,
                max_tickets=6,
            )
            ...
            _v273_trial_lines = _v273_format_three_layer_trial_block(_v273_trial_plan)
            if _v273_trial_lines:
                # 先頭の「パターン」を展開評価の直後へ置き、その後に流れ比率・三層を表示する。
                lines.append(_v273_trial_lines[0])
                lines.append("")
            ...
            if _v273_trial_lines:
                lines.extend(_v273_trial_lines[2:])
                lines.append("")
...
try:
    ...
    # v276：全体妙味は表示せず、パターンA～Eを展開評価の直後へ表示する。
...
def _replace_tanpyou_with_simple_comment(text: str) -> str:
    try:
        ...
        # パターンを取得
        m_pattern = re.search(r"パターン：([A-E])", txt)
        pattern = m_pattern.group(1).strip() if m_pattern else "未判定"
        ...
        line1 = f"・パターン：{pattern}。" if pattern != "未判定" else "・パターン：未判定。"
