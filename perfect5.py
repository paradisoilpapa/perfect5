...
wind_coefficients = {
    ...
    "上": +0.25,
    "右上": +0.15,
    "左": -0.05,   # 左風はやや不利（ホーム直線で外から押される）
    "右": +0.05,   # 右風はやや有利（バック直線で外に膨らみやすい）
    "左下": -0.15,
    "下": -0.25,
    "右下": -0.15
}
...
if st.button("スコア計算実行"):
    ...
    def score_from_tenscore_list(tenscore_list):
        ...
        for score in tenscore_list:
            ...
            correction = {-3: 0.1, -2: 0.125, -1: 0.15, 0: 0.175, 1: 0.2, 2: 0.225}.get(4 - rank, 0.25)
    ...
    def bank_length_adjust(kaku, length):
        ...
        return {'逃': 0.2 * delta, '追': 0.5 * delta, '両': 0.3}.get(kaku, 0.0)
