# v335dp（開催日KO軸入替のみ・5点ミックス修正版）
# ・v335doの重大修正。開催日KO最終順位の母体を「調整後ヒモ順位」から元の最終着順予想へ戻す。
# ・開催日KOは従来仕様どおり元V1と元V2の軸入替判定だけを行い、3位以下の順位は変更しない。
# ・購入順位は「元の最終着順予想＋必要時のみV1/V2入替」で確定する。
# ・その確定順位1→2→3→4→5に対し、2車単1→3／1→4、3連単1→2→3／4／5の計5点。
# ・調整後ヒモ順位、ポイント、加重評価、ライン復元は検証表示用として残すが、開催日KO後の購入順位には混入させない。
# ・V1/V2比較条件、疲労補正、80-90%比率、確率モデル、詳細加重2車単評価表は変更しない。
...
# v335dj（詳細加重2車単評価表版）
# ・v335diの3段階選抜＋保険枠4点ロジックは変更しない。
# ・詳細出力の【加重2車複評価表】を非表示にし、最終軸→各相手の【加重2車単評価表】を表示する。
# ・2車単表は、既存の加重ペア評価（的中点・妙味点・総合点）に、順序付き想定的中率を併記する。
# ・2車複の加重計算自体は第2・第3選考の内部材料として維持し、note用簡易出力には評価表を追加しない。
# v335di（3段階選抜・的中点1位保険復活・2車単4点版）
# ・v335dhを原本に、3段階選抜の妙味3車へ「第2選考時の的中点1位」を保険枠として1車復活し、原則4点とする。
# ・第1選考：調整後総合Pが0以上を通過。3車未満ならマイナスP側から総合P上位を補充して3車確保。
# ・第2選考：第1選考候補を、既存の加重2車複評価表「総合点」で並べて上位3車を暫定採用。
# ・第3選考：第1選考候補が4車以上なら、第2選考3車の「的中点1位」を外し、総合点4位を繰り上げて妙味3車を作る。
# ・保険枠：第3選考で外した的中点1位を1車だけ復活し、妙味3車＋保険1車の4点とする。候補不足時は残り評価上位から補充する。
# ・note上部の「最終着順予想」は、購入判断に実際に使う開催日KO後の最終順位を表示する。元V順位は候補選定欄の「今回V評価順位」に残す。
...
# ・ゆがみヒモが2車以上ある場合は、従来どおりそのゆがみヒモをすべて2車単で購入する。
# ・3連単は軸1着固定の「軸-2車-3車」配列を維持する。
# ・ヒモ側にゆがみ車がある場合は、2列目2車のうち最低1車をゆがみ車とし、残りをV評価上位から選ぶ。
# ・ヒモ側にゆがみ車がない場合は、V評価上位を使って「◎-12-123」を組む。
# ・3列目は2列目2車に、残るV評価上位1車を追加する。
# ・各2車単/3連単の想定的中率表示、軸1着想定、候補選定表示は維持する。
# v335ci（2車単ゆがみ専用・3連単ゆがみ必須配列版）:
# ・2車単はv335chどおり、軸=V最終着順予想1位固定、ヒモはゆがみポイント>0の車だけを購入する。
# ・3連単は軸1着固定の「軸-2車-3車」配列へ戻す。
# ・3連単2列目には必ずゆがみ車を最低1車入れる。必須ゆがみ車はゆがみポイント降順、同点はV順位上位。
# ・3連単2列目の残り1車は、足切り通過候補（上昇+据え置き）のうちV順位最上位を採用する。
# ・3連単3列目は2列目2車に、残る足切り通過候補のV順位最上位1車を追加する。据え置き/ゆがみは問わない。
# ・ゆがみ車が0車、または軸以外の足切り通過候補が3車未満なら3連単は「なし」。
# ・各2車単/3連単の想定的中率表示、軸1着想定、最終着順予想、足切り判定は変更しない。
# v335ch（ゆがみのみ購入・買目想定的中率表示版）:
# ・軸はヴェロビ最終着順予想1位で固定。軸選定ロジックは変更しない。
# ・購入ヒモは「ゆがみポイント=過去2着内率順位-V評価順位」が+1以上の車だけに絞る。据え置き0は買わない。
# ・2車単は軸→ゆがみヒモを元のV評価順位順で表示する。
# ・3連単も軸1着固定を維持し、ゆがみ候補が2車以上ある場合のみ、ゆがみ上位2車を2着、上位3車を3着候補にする。
# ・2車単／3連単の実際の各買い目ごとに、既存v335brモデルの順序付き想定的中率を表示する。
# ・過去順位と今回V順位は従来どおり表示し、読者向け候補欄は実際に買う「ゆがみヒモ候補」を表示する。
# ・1着想定・買目想定的中率はいずれもヴェロビ内部指標のモデル値であり、実測的中率ではない。
# v335cf（候補選定インデント整理版）:
...
# ・Aパターン補足の「方向」を「買い目」に表記修正する。
# v335bg（Aパターン・2車単追加購入ルール注記版）:
# ・2車複を2車単へ変換した際、2車複オッズの3倍以上となる方向があれば、その2車単を100円追加購入する旨をnote表示へ追記。
# v335bf（資金目安を補足行へ移動版）:
...
# ・3連単は評価順位23-123-12（231／312／321の3点）。
# ・2車単は評価順位3-124（3→1／3→2／3→4の3点）。
# ・苦手会場ファジー3連複と最終日3連複への券種分岐を廃止する。
# ・最終着順選別はv335bbの50:50統合ロジックを全会場共通で使用する。
# ・市場◎回避および地区まとめ3番手の1着禁止は3連単・2車単には適用しない。
...
# ・2車単は専用順位2位・3位・4位→専用順位1位の「234-1」3点固定。三連複の除外条件は2車単順位生成には適用しない。
# ・3連単は現行23-13-123の3点を維持。3連複も現行のTOP5候補＋除外ロジックを維持する。
# ・バンク形状2条件以上は「2車単3点＋3連単」、0～1条件は「2車単3点＋3連複」。最終日も同じ共通ロジックを使う。
# ・最終日専用2車単／2車複ロジックは推奨購入から廃止する。
# -*- coding: utf-8 -*-
...
# ・対象条件内かつ8～9月集計の回収率80%以上の会場を推奨A、80%未満を推奨B、それ以外を推奨Cとする。
# ・条件内だが回収率未分類の会場は推奨C（条件合致・回収率未分類）として表示する。
...
# ・従来の三連複的中点順位1～5位は第3候補Cを決める内部材料として維持する。
# ・note用簡易表示から「三連複的中点順位想定」は外し、推奨5点だけを表示する。
...
# ・両想定の和集合全体で的中点1位と総合点1位が一致した1点だけ200円とする。
# ・既存の三連複ロジック、詳細評価表、note簡易版を残したまま2車複推奨を追加する。
# v334p（note想定6点表示版）:
...
def _v335cd_pass_candidates(final_order, profile):
    """
    v335cd 足切り候補。
    V評価順位 <= 過去車番別2着内率順位 の車を残す。
    つまり上昇車・据え置き車を残し、下落車だけを除外する。
    戻り値: (V順位順の通過候補, 過去順位map, V順位map, ゆがみポイントmap)
    """
    _order = tuple(int(x) for x in (final_order or tuple()))
    _v_rank = {int(c): i for i, c in enumerate(_order, start=1)}
    _hist_rank = _v335cc_historical_quinella_rank_map(profile, _order)
    ...
    for _car in _order:
        _car = int(_car)
        _vr = _v_rank.get(_car)
        _hr = _hist_rank.get(_car)
        if _vr is None or _hr is None:
            continue
        _point[_car] = int(_hr) - int(_vr)
        if int(_vr) <= int(_hr):
            _passed.append(_car)

    return _passed, _hist_rank, _v_rank, _point


def _v335cd_trifecta_12_13_123(cars3):
    """
    順位1・2・3の3車から、12-13-123 の有効3点を生成する。
    例: [A,B,C] -> A-C-B / B-A-C / B-C-A
    """
    _cars = [int(x) for x in (cars3 or [])]
    if len(_cars) < 3:
        return []
    _a, _b, _c = _cars[:3]
    return [
...
def _v335bt_purchase_lines(final_order, profile):
    """
    v335dp note用推奨購入:
      1) 元の最終着順予想を母体にし、開催日KOはV1/V2の軸入替だけを反映
      2) 確定順位1位を頭として、2車単1→3／1→4を購入
      3) 同じ確定順位1位→2位を1・2着固定し、3着3位／4位／5位の3連単3点を購入
      4) 合計5点固定（2車単2点＋3連単3点）
      5) 既存のポイント・加重評価・ライン復元は検証表示用として維持し、購入選定には使用しない
      6) note上部の最終着順予想は元順位へV1/V2のKO入替だけ反映した確定順位を表示
    ...
    # v335dp：開催日KOの母体は「元の最終着順予想」そのもの。
    # 調整後ヒモ順位は購入順位へ混入させない。
    # 開催日KOは従来仕様どおり元V1と元V2の軸入替だけを判定し、3位以下は元順位を維持する。
    _pre_knock_order = [int(c) for c in _order]
    ...
    # v335dp：note上部の「最終着順予想」には、元順位へV1/V2のKO入替だけ反映した確定順位を渡す。
    globals()["V335DI_KNOCK_FINAL_ORDER"] = tuple(int(c) for c in _knock_order)
    ...
    def _v335dc_restore_himo_lines(_base_order, _axis_car, _line_def_map):
        """
        v335df2:
        ・軸ラインから、開催日KO最終順位が最上位の1車を必ず確保。
        ・2車ラインなら相方がKO4位以下でも確保。
        ・残り2枠は開催日KO最終順位上位から採用。
        ・軸単騎ならKO上位3車。
        """
        _axis = int(_axis_car)
        _base = [int(c) for c in (_base_order or []) if int(c) != _axis]
        ...
        # ① 軸ラインからKO順位最上位の1車を必ず保護
        _axis_members = [
            int(x) for x in _car_line.get(_axis, [_axis])
            if int(x) != _axis
        ]
        if _axis_members:
    ...
    # v335di：2車単ヒモは「妙味3車＋的中点1位の保険1車」で原則4点。
    # 第1選考：
    #   ・調整後総合Pが0以上の車を通過。
    ...
    # 第3選考：
    #   ・第1選考候補が4車以上なら、暫定3車のうち的中点1位を除外し、
    #     加重2車複総合点4位を繰り上げて「妙味3車」を作る。
    # 保険枠：
    #   ・第3選考で外した的中点1位を1車だけ復活して4点化する。
    #   ・候補3車しかない等で的中点1位が妙味3車内に残る場合は、残り評価上位から1車補充して4点を維持。
    ...
    _weighted_pair_rows = list(globals().get("V335DI_WEIGHTED_PAIR_ROWS", []) or [])
    _axis_pair_metric = {}
    ...
    def _v335di_pair_sort_key(_car):
    ...
    _stage1_weighted_order = sorted(
        ...
        key=_v335di_pair_sort_key,
    )

    # ---- 第2選考：加重2車複総合点上位3車 ----
    _stage2_top3 = [int(c) for c in _stage1_weighted_order[:3]]

    # 第2選考3車の的中点1位。第3選考で外した場合も、最後に保険枠として復活させる。
    _stage2_hit_top = None
    if _stage2_top3:
        _stage2_hit_top = max(
            _stage2_top3,
            key=lambda c: (
    ...
    # ---- 第3選考：的中点1位を外し、総合点4位を繰上げて妙味3車 ----
    _stage3_removed_hit_top = None
    _stage3_promoted_total4 = None
    _stage3_value3 = list(_stage2_top3)

    if len(_stage1_weighted_order) >= 4 and len(_stage2_top3) == 3 and _stage2_hit_top is not None:
        _stage3_removed_hit_top = int(_stage2_hit_top)
        _stage3_promoted_total4 = int(_stage1_weighted_order[3])
        _stage3_value3 = [
            int(c) for c in _stage2_top3
            if int(c) != int(_stage3_removed_hit_top)
        ]
        if int(_stage3_promoted_total4) not in _stage3_value3:
            _stage3_value3.append(int(_stage3_promoted_total4))
        _stage3_value3 = sorted(_stage3_value3, key=_v335di_pair_sort_key)[:3]

    # 妙味3車が不足した場合は、第1選考順位→KO後順位の順で3車まで補充。
    if len(_stage3_value3) < 3:
        for _c in _stage1_weighted_order:
            if int(_c) not in _stage3_value3:
                _stage3_value3.append(int(_c))
            if len(_stage3_value3) >= 3:
                break
    if len(_stage3_value3) < 3:
        for _c in _post_himo_order:
            ...
            if len(_stage3_value3) >= 3:
                break
    _stage3_value3 = [int(c) for c in _stage3_value3[:3]]

    # ---- 保険枠：第2選考時の的中点1位を1車だけ復活 ----
    _insurance_hit_top = int(_stage2_hit_top) if _stage2_hit_top is not None else None
    _himo = list(_stage3_value3)
    if _insurance_hit_top is not None and int(_insurance_hit_top) not in _himo:
        _himo.append(int(_insurance_hit_top))

    # 的中点1位が妙味3車内に残っていて4車にならない場合は、
    # 第1選考の次点→除外済み車を含む全残りの加重総合点順で1車補充する。
    if len(_himo) < 4:
        _all_remaining_weighted = sorted(
            [int(c) for c in _post_himo_order if int(c) not in _himo],
            key=_v335di_pair_sort_key,
        )
        for _c in _all_remaining_weighted:
    ...
    # v335dp：実購入は「元の最終着順予想＋必要時のみV1/V2入替」の確定順位で固定5点を作る。
    # 確定順位 1→2→3→4→5→6→7 に対し、
    #   2車単：1→3、1→4
    ...
    # 表示対象は最終軸→軸以外の全車。
    # 的中点・妙味点・総合点は、選抜で実際に使っている既存加重ペア評価をそのまま使用し、
    # 2車単としての方向性は順序付き想定的中率を併記して確認する。
    _v335dj_exacta_rows = []
    for _c in _post_himo_order:
        try:
            _c = int(_c)
            _m = dict(_axis_pair_metric.get(_c, {}) or {})
            _prob = float(_v335br_ticket_probability((int(_axis), _c), _p1_map, _p2_map, _p3_map))
            _v335dj_exacta_rows.append({
                "disp": f"{int(_axis)}-{_c}",
                "axis": int(_axis),
                "himo": _c,
                "hit_score": float(_m.get("hit_score", 0.0) or 0.0),
                "myoumi_score": float(_m.get("myoumi_score", 0.0) or 0.0),
                "total_pt": float(_m.get("total_pt", 0.0) or 0.0),
                "hit_prob": _prob,
            })
        except Exception:
            pass
    _v335dj_exacta_rows = sorted(
        _v335dj_exacta_rows,
        key=lambda _r: (
            float((_r or {}).get("total_pt", 0.0) or 0.0),
            float((_r or {}).get("hit_score", 0.0) or 0.0),
            float((_r or {}).get("myoumi_score", 0.0) or 0.0),
            float((_r or {}).get("hit_prob", 0.0) or 0.0),
        ),
        reverse=True,
    )
    globals()["V335DJ_WEIGHTED_EXACTA_ROWS"] = [dict(_r) for _r in _v335dj_exacta_rows]

    _tri_tickets = []
    for _b in _tri_second:
        for _c in _tri_third:
            if int(_b) == int(_c):
                continue
            _tri_tickets.append((int(_axis), int(_b), int(_c)))
    _tri_probs = {
        tuple(t): float(_v335br_ticket_probability(tuple(t), _p1_map, _p2_map, _p3_map))
        for t in _tri_tickets
    }
    ...
    if not _ko_purchase_ready:
        _out.append("2車単・3連単：確定順位5位まで不足のため算出不可")
    else:
        _out.append(
...
def _v334q_build_axis_pair_tickets(axis_car, selected_five):
    """軸1車と同じ想定の残り4車から、2車複の基本4点を作る。"""
    try:
        axis = int(axis_car)
        partners = []
        for car in (selected_five or []):
            try:
                car_int = int(car)
            except Exception:
                continue
            if car_int == axis or car_int in partners:
                continue
            partners.append(car_int)
        if axis <= 0 or len(partners) != 4:
            return tuple()
        return tuple(
            tuple(sorted((axis, int(partner))))
...
def _v334r_build_pair_top1_union_purchase_plan(basic_pairs, weighted_pair_rows):
    ...
    try:
        ...
        for pair in (basic_pairs or []):
            try:
                if isinstance(pair, (tuple, list)):
                    ...
                    key = tuple(sorted(
                        int(x) for x in re.findall(r"[1-9]", str(pair))
                    ))
                if len(key) == 2 and len(set(key)) == 2 and key not in basic:
                    basic.append(key)
            except Exception:
                pass
        if len(basic) != 4:
        ...
        basic_order = {key: idx for idx, key in enumerate(basic)}
        row_map = {}
        for row in (weighted_pair_rows or []):
            try:
                ...
                if len(key) == 2 and key in basic_order:
                    row_map[key] = row
            except Exception:
                pass
...
def _v334q_merge_pair_axis_purchase_plans(*plans):
    """想定A/Bの2車複を統合し、的中・総合1位一致の最大1点だけ200円にする。"""
    selected = set()
    score_map = {}
    for plan in plans:
        if not isinstance(plan, dict) or not plan:
    ...
    stakes = tuple(
        (key, 200 if overlap_first == key else 100)
        for key in selected_sorted
    )
    return {
        "selected": tuple(selected_sorted),
        ...
        "hit_first": tuple(hit_ranked[0]["key"]) if hit_ranked else tuple(),
        "total_first": tuple(total_ranked[0]["key"]) if total_ranked else tuple(),
        "overlap_first": tuple(overlap_first),
        "stakes": stakes,
        "ticket_count": len(selected_sorted),
        "total_amount": sum(int(amount) for _, amount in stakes),
    }
...
def _v335k_build_top12_five_point_plan(plan, trio_plan):
    ...
    ・最多出現車をCへ採用する。同数なら着順予想上位、さらに同順位なら車番順で決める。
    ...
    try:
        ...
        if len(adopted_sequence) < 3 or len(ranked_trios) < 5:
            return {}
        ...
        third = int(candidate_order[0])

        # 旧デバッグ互換用の上位3候補。3車未満でも第3候補Cさえ確定すれば新推奨は生成できる。
        ...
        if not isinstance(rating_map, dict) or not rating_map:
            rating_map = globals().get("tenscore", globals().get("tenscores", {})) or {}

        score_pairs = []
        for car in adopted_sequence:
            try:
                raw = rating_map.get(int(car), rating_map.get(str(car), None))
                if raw is None:
                    continue
                val = float(raw)
                if np.isfinite(val):
                    score_pairs.append((int(car), val))
            except Exception:
                continue
        score_pairs.sort(key=lambda x: (-float(x[1]), int(x[0])))
        score_top3 = (
            {int(car) for car, _ in score_pairs[:3]}
        ...
        trio_excluded = []
        seen_trios = set()
        for rank_idx, key in enumerate(ranked_trios[:5], start=1):
            combo = tuple(sorted(int(x) for x in (key or tuple())))
            if len(combo) != 3 or combo in seen_trios:
                continue
            seen_trios.add(combo)

            reasons = _trio_exclusion_reasons(combo)
            if reasons:
                trio_excluded.append({
                    "rank": int(rank_idx),
                    "ticket": combo,
        ...
        common_exacta_source_trios = []
        _common_exacta_seen_source = set()
        for _key in ranked_trios[:5]:
            _combo = tuple(sorted(int(x) for x in (_key or tuple())))
            if len(_combo) != 3 or _combo in _common_exacta_seen_source:
                continue
            _common_exacta_seen_source.add(_combo)
            common_exacta_source_trios.append(_combo)
            for _raw_car in _combo:
                _car = int(_raw_car)
                common_exacta_counts[_car] = int(common_exacta_counts.get(_car, 0)) + 1

        # TOP5に一度も出ない車も0回として末尾に残し、常に全車の専用順位を作る。
        ...
        # v335bc：会場評価A/B/C/Dに関係なく、最終着順選別は全会場で同一。
        common_exacta_order = tuple(int(x) for x in merged_final_order)
        ...
        validation_exacta_tickets = common_exacta_tickets
        validation_exacta_text = common_exacta_text
        ...
        if len(final_prediction_order) >= 5:
            ...
            for _fz_head in _fz_top:
                for _fz_low in fuzzy_lower_two:
                    _combo = tuple(sorted((_fz_head, _fz_mid, _fz_low)))
                    if len(set(_combo)) == 3 and _combo not in _fz_seen:
                        _fz_seen.add(_combo)
                        _fz_rows.append(_combo)
            fuzzy_trio_tickets = tuple(_fz_rows)
            fuzzy_trio_text = "・".join(
                "".join(str(int(x)) for x in _combo)
                for _combo in fuzzy_trio_tickets
        ...
        return {
            ...
            "exacta_tickets": tuple(),
            "quinella_ticket": tuple(),
            "exacta_text": "",
            "quinella_text": "",
            ...
            "recommended_exacta_tickets": common_exacta_tickets,
            "recommended_exacta_text": common_exacta_text,
            ...
            "fuzzy_lower_two": fuzzy_lower_two,
            "fuzzy_trio_tickets": fuzzy_trio_tickets,
            ...
            "final_day_quinella_heads": final_quinella_heads,
            "final_day_quinella_himo_candidates": final_quinella_himo_candidates,
            "final_day_quinella_spilled_jiryoku": final_quinella_spilled_jiryoku,
            "final_day_quinella_spilled_jiryoku_candidate": final_quinella_spilled_jiryoku_candidate,
            "final_day_quinella_non_candidate_pool": final_quinella_non_candidate_pool,
            "final_day_quinella_non_candidate": final_quinella_non_candidate,
            ...
            "validation_alt_tickets": trio_tickets,
            "validation_alt_text": trio_text,
            "validation_alt_type": "旧3連複",
...
def _v334n_build_compact_note_text(plan, weighted_trio_rows, queue_source=""):
    ...
    try:
        ...
        # v335s:
        # ・三連複的中点順位1～5位は第3候補Cの選定と三連複購入候補に使用する。
        ...
        if five_point_plan:
            trifecta_text = str(
                five_point_plan.get("trifecta_text", "算出不可") or "算出不可"
            )
            trio_text = str(
        ...
        if _hit_top_lines:
            # v335di：上部の最終着順予想は、購入判断で実際に使った開催日KO後順位へ統一する。
            _ko_final_order = tuple(
                int(x) for x in (globals().get("V335DI_KNOCK_FINAL_ORDER", tuple()) or tuple())
            )
            if _ko_final_order and len(lines) >= 4:
                lines[3] = f"最終着順予想　{' → '.join(str(int(x)) for x in _ko_final_order)}"
            lines.append("")
        ...
        pair_snapshot = dict((purchase_snapshot.get("pair", {}) or {}))
        pair_a_only = [
            tuple(sorted(int(x) for x in (key or tuple())))
            for key in (pair_snapshot.get("a_only", tuple()) or tuple())
        ]
        pair_e_only = [
            tuple(sorted(int(x) for x in (key or tuple())))
            for key in (pair_snapshot.get("e_only", tuple()) or tuple())
        ]
        pair_common = [
            tuple(sorted(int(x) for x in (key or tuple())))
            for key in (pair_snapshot.get("common", tuple()) or tuple())
        ]
        pair_stake_map = {
            tuple(sorted(int(x) for x in (key or tuple()))): int(amount)
            for key, amount in (pair_snapshot.get("stakes", tuple()) or tuple())
        }
        pair_ticket_count = int(pair_snapshot.get("ticket_count", 0) or 0)
        pair_total_amount = int(pair_snapshot.get("total_amount", 0) or 0)
        ...
        def _pair_group_text(keys):
            parts = [
                f"{'-'.join(str(int(x)) for x in key)}（{int(pair_stake_map.get(key, 100))}円）"
                for key in (keys or [])
            ]
            return " ".join(parts) if parts else "なし"
...
def _v281_format_fixed_flow_block(
    plan,
    weighted_trio_rows=None,
    weighted_pair_rows=None,
    mark_map=None,
):
    if not isinstance(plan, dict) or not plan:
        return []
    ...
    if all_line_snapshot:
        ...
        if five_point_plan:
            trifecta_counts = dict(five_point_plan.get("trifecta_counts", {}) or {})
            trifecta_order = tuple(
                five_point_plan.get("trifecta_candidate_order", tuple()) or tuple()
            )
            count_text = " ".join(
                f"{int(car)}={int(trifecta_counts.get(int(car), 0))}回"
                for car in trifecta_order[:3]
            ) or "算出不可"
            _ta = int(five_point_plan.get("trifecta_axis_a", 0) or 0)
            _tb = int(five_point_plan.get("trifecta_axis_b", 0) or 0)
            _excluded = tuple(
            ...
            _trio_excluded_rows = tuple(
                five_point_plan.get("trio_excluded", tuple()) or tuple()
            )
            _trio_excluded_text = " ".join(
                f"{''.join(str(int(x)) for x in (row.get('ticket', tuple()) or tuple()))}"
...
def _make_note_final_summary_block(rec_style, rec_seq, mark_map=None):
    ...
    """
    # v335di：レース切替時に前レースの加重2車複評価／KO最終順位を持ち越さない。
    globals()["V335DI_WEIGHTED_PAIR_ROWS"] = []
    globals()["V335DI_KNOCK_FINAL_ORDER"] = tuple()
    globals()["V335DJ_WEIGHTED_EXACTA_ROWS"] = []
    try:
        xs = []
        seen = set()
        for x in (rec_seq or []):
        ...
        if flow_items:
            ...
            # v335di：compact note側の3段階＋保険枠ヒモ選抜で、既存の加重2車複評価結果をそのまま再利用する。
            globals()["V335DI_WEIGHTED_PAIR_ROWS"] = [
            ...
            # v231:
            # 旧加重2車複評価表の整形関数は内部互換のため残すが、v335djでは詳細表示しない。
...
# v335dj：加重2車単評価表は「詳細出力」だけに表示する。note用簡易出力には追加しない。
try:
    _v335dj_rows = list(globals().get("V335DJ_WEIGHTED_EXACTA_ROWS", []) or [])
    if _v335dj_rows:
        def _v335dj_fmt_num(_v):
            try:
                return f"{float(_v):.1f}"
            except Exception:
                return "-"
        ...
        _v335dj_lines = [
            "【加重2車単評価表】",
            "　買い目　　 的中点　 妙味点　 総合点　 想定的中率",
        ]
        for _r in _v335dj_rows:
            try:
                _disp = str((_r or {}).get("disp", "")).strip()
                if not _disp:
                    continue
                _hit = _v335dj_fmt_num((_r or {}).get("hit_score", 0.0))
                _myo = _v335dj_fmt_num((_r or {}).get("myoumi_score", 0.0))
                _tot = _v335dj_fmt_num((_r or {}).get("total_pt", 0.0))
                _prob = float((_r or {}).get("hit_prob", 0.0) or 0.0) * 100.0
                _v335dj_lines.append(
                    f"　{_disp:<3}　　　　{_hit:>3}　　　{_myo:>3}　　　{_tot:>3}　　　{_prob:>5.1f}%"
                )
            except Exception:
                pass
        _v335dj_block = "\n".join(_v335dj_lines).strip()
        if _v335dj_block:
            _marker = "【加重3連複評価表】"
            if _marker in note_text:
                note_text = note_text.replace(_marker, _v335dj_block + "\n\n" + _marker, 1)
            elif "＜短評＞" in note_text:
                note_text = note_text.replace("＜短評＞", _v335dj_block + "\n\n＜短評＞", 1)
            else:
                note_text = note_text.rstrip() + "\n\n" + _v335dj_block
except Exception as _e:
    st.caption(f"詳細・加重2車単評価表生成不可：{_e}")

st.text_area("ここを選択してコピー", compact_note_text, height=460)
