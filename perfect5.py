...
# ・ただしこの検証版だけは、フォーメーション作成時のAライン全車保護、採用核ライン全車保護、
#   核ライン成立のためのC固定、別線C補強、全ライン代表最大化を使わない。
#   A/B決定後の残り3車は採用流れ着順順で選び、C/D/Eの列配置は実績着内率だけで決める。
...
def _v281_build_fixed_flow_plan(
    ...
    # v322検証：ライン保護は役割に使わない。A/B以外は採用流れ順位で選ぶ。
    same_line_himo = []
    forced_external_c = None
    forced_single_core_c = None
